%\VignetteIndexEntry{Rectangular Statistical Cartograms R (JSS submission #2711)}
...
%% almost as usual
\author{Christian Panse\\Swiss Federal Institute of Technology in Zurich }

\title{Rectangular Statistical Cartograms in \proglang{R}: The \pkg{recmap} Package\thanks{This vignette is a (slightly) modified version of jss2711, to be published in the Journal of Statistical Software.}}
...
\SweaveOpts{prefix.string = graphics/recmap, eval = TRUE}
...
dir.create("graphics")
message("This manuscript requires the R packages: recmap (>=0.5.29), maps, noncensus.")
message(paste("recmap version", packageVersion('recmap'), "is used."))


stopifnot(packageVersion('recmap') >= '0.5.29')
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth,keepaspectratio]{graphics/recmap-us_election2004}
...
\resizebox{1.0\textwidth}{!}{
	\includegraphics{graphics/recmap-usage}
}
...
\begin{figure}[htbp]
  \centering
\includegraphics[width=0.49\textwidth]{graphics/recmap-bearing}
\includegraphics[width=0.49\textwidth]{graphics/recmap-placement}
...
For a performance study the checkerboard bench described above was repeated
using the \pkg{rbenchmark} package by \cite{rbenchmark} to measure the \code{proc.time}. The
study was performed on two systems (using one core only). An Intel(R) Core(TM)
i5-2500 CPU @ 3.30GHz running a Debian 8 using a 3.16.0-4-amd64 Kernel and a
3 GHz Intel Core i7 (Apple MacBook Pro) running OS X 10.11.4 (15E65) using a
Darwin 15.4.0 Kernel. The middle graphic in Figure~\ref{figure:benchmark}
displays the resulting mean aggregated measured data of the two (hardware /
OS) systems using the two different implementations of the MBB intersection
...
################################################################################
x <- c(2:20)
maci7_2.5 <- c(0.189, 0.184, 0.194, 0.237, 0.295, 0.423, 0.65, 0.973, 1.405,
	2.318, 3.648, 6.05, 7.661, 9.076, 14.27, 18.264, 23.075, 27.5, 36.388) / 100

maci7_2.1 <- c(0.175, 0.184, 0.208, 0.337, 0.465, 0.746, 1.291, 1.925, 2.901,
	4.71, 7.466, 11.496, 15.215, 20.157, 26.262, 33.669, 50.556, 55.932, 67.337) / 100

linuxi5_2.1 <- c(0.176, 0.26, 0.372, 0.632, 0.904, 1.416, 2.176, 3.016, 4.08, 6.14,
	8.604, 11.996, 15.384, 19.192, 25.448, 32.74, 39.54, 46.392, 60.116) / 100

linuxi5_2.5 <- c(0.192, 0.28, 0.38, 0.64, 0.892, 1.372, 2.136, 3.008, 4.1, 6.3,
	8.892, 12.5, 16.768, 21.092, 28.112, 36.772, 43.816, 52.036, 69.66) / 100

plot(x, linuxi5_2.5,
    xlab = 'number of map regions',
    ylim = range(c(maci7_2.5, maci7_2.1, linuxi5_2.5 , linuxi5_2.1)),
    ylab = 'mean $user.self [in seconds]', type='b',
    pch = "L",
    axes = FALSE,
    log = '',
    lwd = 2)
abline(a = 0, b = 1, col = 'lightgrey')
lines(x, linuxi5_2.1, col = 'red', pch = "L", type = 'b', lwd = 2)

lines(x, maci7_2.5, col = 'black', pch = "A", type = 'b',lwd = 1)
lines(x, maci7_2.1, col = 'red', pch = "A", type = 'b',lwd = 1)
legend("topleft", c("recmap 0.3.0 MacOSX i7", "recmap 0.3.0 Debian i5",
	"recmap 0.2.1 MacOSX i7", "recmap 0.2.1 Debian i5"),
	col=c('black', 'black', 'red', 'red'), pch=c('A', 'L', 'A', 'L'), cex = 1.5)
axis(1, c(5, 10, 15, 20), c("5x5", "10x10", "15x15", "20x20"))
axis(2)
box()

################################################################################
plot(x, round(1 / maci7_2.5),
	xlab = 'number of map regions',
...
points(x, round(1 / maci7_2.5),
	pch = 22)

legend("topright", c("recmap 0.3.0 MacOSX i7"), pch = 22, cex = 1.5)
...
\begin{figure}[htbp]
  \centering
\includegraphics[width=1.0\textwidth]{graphics/recmap-benchmark}
...
<<eval=FALSE>>=
set.seed(1)
res.GRASP <- recmapGRASP(Checkerboard)
...
higher than 1.7. Having reached a fitness value of 1.7
...
Checkerboard <- checkerboard(8)
#plot(Checkerboard <- checkerboard(8),
#            col=c('white','white','white','black')[Checkerboard$z])

# found by the GA
smp <- c(52, 10, 27, 63, 7, 20, 32, 18, 47, 28, 6, 55, 11, 61, 38,
50, 5, 21, 36, 34, 2, 22, 3, 1, 29, 57, 43, 4, 51, 58, 31, 49, 44,
25, 59, 33, 17, 40, 8, 41, 26, 37, 19, 56, 45, 35, 62, 53, 24, 64,
30, 15, 39, 12, 60, 48, 16, 23, 46, 42, 13, 54, 14, 9)

Cartogram.Checkerboard <- recmap(Checkerboard[smp, ])
...
lines(Cartogram.Checkerboard$x[idx], Cartogram.Checkerboard$y[idx], col = 'red', lwd = 2)
...
op <- par(mar = c(4, 4, 0.5, 0.5), mfrow = c(1, 1), bg = 'white')
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1.0\textwidth,keepaspectratio]{graphics/recmap-CB8_GRASP_GA}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.49\textwidth,keepaspectratio]{graphics/recmap-CB8}
\includegraphics[height=0.49\textwidth,keepaspectratio]{graphics/recmap-cmp_GA_GRASP}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth,keepaspectratio]{graphics/recmap-GAseed}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth,keepaspectratio]{graphics/recmap-bestSolutionFitness}
\includegraphics[width=0.85\textwidth,keepaspectratio]{graphics/recmap-bestSolution}
...
function(input, Map = US.map, DF = state.x77,
  ...
  res <- recmapGA(Map = Map,
    popSize = 300, maxiter = 30, run = 10,
    parallel = RECMAP_ENABLE_PARALLEL)
  ...
  res$time.elapsed = (proc.time() - ptm)[3]
  res
}
@

As input map the \code{US.map} defined in Section~\ref{section:usage} is used.
An interactive \pkg{shiny}~\citep{shiny} web application,
using this method, provides more combination of
attributes and is available through
\url{https://recmap.shinyapps.io/state_x77/}.

<<label=x77, fig = TRUE, width=8, height=16, echo=TRUE, keep.source = FALSE, include=FALSE>>=
op <- par(mfrow = c(4, 1), mar = rep(0.25, 4), bg = 'white')
set.seed(1)
cartogram.x77 <- lapply(list(list(color="Area", area="Population"),
...
res <- lapply(cartogram.x77, function(x){plot(x$GA)})
par(op)
@
    
\begin{figure}[htbp]
\centering
\includegraphics[height=0.9\textheight,keepaspectratio]{graphics/recmap-x77}
\includegraphics[height=0.9\textheight,keepaspectratio]{graphics/recmap-x77GA}
\caption{Rectangular Statistical Cartograms using the 'US State Facts
and Figures' dataset are drawn (see also
\url{https://recmap.shinyapps.io/state_x77/}).
...
# install.packages(c('maps', 'noncensus'))
data("counties")
...
recmap_ga_county <- function(state='colorado'){
...
ptm <- proc.time()
  ...
  recmapGA <- ga(type = "permutation",
    ...
    plot(Cartogram <- recmap(M[recmapGA@solution[1, ], ]), col.text = 'darkred')
    summary(Cartogram)
	...
	Cartogram = Cartogram, 
	state = state, 
	time.elapsed = (proc.time() - ptm)[3])
}

set.seed(1)
op <- par(mfrow = c(5, 2), mar = c(0, 0, 0, 0), bg = 'white')
res.us_county <- lapply(c('california',
    'colorado',
    'florida',
    'new jersey',
    'new york'),
...
res <- lapply(res.us_county, function(x){plot(x$GA)})
par(op)
@

\begin{figure}[htbp]
\centering
\includegraphics[height=0.8\textheight, keepaspectratio]{graphics/recmap-us_counties}
\includegraphics[height=0.8\textheight, keepaspectratio]{graphics/recmap-us_countiesGA}
...
There are 2300 rectangles to place. The statistical
...
swisstopo map. On the web application, available through
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1.0\textwidth, keepaspectratio]{graphics/recmap-Switzerland_population}
...
of all 400 geo-locations which define the pseudo dual of the map.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1.0\textwidth, keepaspectratio]{graphics/recmap-sbb}
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.25\textwidth, keepaspectratio]{graphics/recmap-UK_Map}
\includegraphics[height=0.25\textwidth, keepaspectratio]{graphics/recmap-UK_spplot}
\includegraphics[width=0.8\textwidth, keepaspectratio]{graphics/recmap-UK_Cartogram}
...
Table~\ref{table:overview} provides an overview of the 
