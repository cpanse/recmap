...
XYZ <- do.call('rbind', list(Z,X,Y))

xt <- xtable(XYZ[order(XYZ['Map.number.regions']),], caption = "\\label{tab:summary}The table lists a Summary of the X77 and U.S. county level examples.")
