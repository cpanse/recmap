...
The results of the recmap implementation are reproducible on the same 
platform however due to the numerical ill-condition special care has to
be taken for computing of the fitness value. Small differences in the
fitness values will cause error propagation over the iterations of the
metaheuristic and derive a different solution sequence $\Pi$ and cartogram
drawing.
...
xt <- xtable(XYZ[order(XYZ['Map.number.regions']),], caption = "\\label{table:reproduced:overview}The spreadsheet provides an overview of the statistical rectangular cartograms drawn in Figures \\ref{figure:cmp_GA_GRASP}, \\ref{figure:bestSolution}, \\ref{figure:x77}, and \\ref{figure:us_census} ordered by the number of map regions.")
