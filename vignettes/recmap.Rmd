...
```{r fig.width=7, fig.retina=2, fig.align='center', fig.cap="Rectangular Cartogram of the U.S. election 2004; The area corresponds to the number of electors (red: demo / blue: rep). The graphic was computed by using the original implementation of the construction heuristic RecMap MP2 introduced in [@recmap].", echo=FALSE, warning=FALSE, comment="ccc", error=FALSE, message=FALSE}
...
library(recmap)
plot.recmap(M <- usa[!usa$name %in% c("Hawaii", "Alaska"), ],  cex=0.5)
#plot.recmap(C<-recmap(M), cex=0.5)
```
...
The local placement function $f: [-\pi,\pi] \rightarrow (\mathbb{R}, \mathbb{R})$ maps an angle into a coordinate.
...
plot.recmap(Cartogram.Population <- recmap(M[order(M$x),]), cex=0.4)
```
...
M <- usa[!usa$name %in% c("Hawaii", "Alaska"), ]
plot.recmap(Cartogram.Income <- recmap(M[order(M$x),]), cex=0.4)
```
...
lapply(c(2,8), function(n){
  ...
  plot.recmap(Map <- recmap:::.checker_board(n))
  plot.recmap(Cartogram <- recmap(Map))
