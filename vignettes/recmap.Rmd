...
library(recmap)
plot_recmap(M <- usa[!usa$name %in% c("Hawaii", "Alaska"), ],  cex=0.5)
...
plot_recmap(Cartogram.Population <- recmap(M[order(M$x),]), cex=0.4)
```
...
M <- usa[!usa$name %in% c("Hawaii", "Alaska"), ]
plot_recmap(Cartogram.Income <- recmap(M[order(M$x),]), cex=0.4)
```
...
lapply(c(2,8), function(n){
  ...
  plot_recmap(Map <- recmap:::.checker_board(n))
  plot_recmap(Cartogram <- recmap(Map))
